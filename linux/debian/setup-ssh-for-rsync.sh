...
# rsync operations between Debian Linux machines. It handles:
...
between Debian Linux machines. It automates the entire process from
...
WHAT THIS SCRIPT DOES:
    1. Checks and installs openssh-client and rsync packages
...
check_dependencies() {
    ...
    if ! command_exists ssh-copy-id; then
        print_warning "ssh-copy-id not found (usually part of openssh-client)"
    fi
    ...
    if [ "$need_install" = true ]; then
        ...
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            ...
            echo ""
            echo "Install manually with: sudo apt-get install ${missing_deps[*]}"
            exit 1
        fi
    else
...
main() {
    ...
    echo ""
    echo "════════════════════════════════════════════════════════════════"
    echo "  SSH Setup for rsync - Debian Linux"
    echo "════════════════════════════════════════════════════════════════"
    echo ""
